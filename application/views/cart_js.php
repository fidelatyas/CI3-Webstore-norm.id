<script type="text/javascript">
		...
		$.ajax({
            ...
            success: function(data) {
                if (data.status == 'success') {
                    ...
                    $('.cart-'+ productId).html(`Add to Cart`);
                    $('.cart-button.cart-'+ productId).html(`<div><img loading="lazy" src="<?= base_url(); ?>assets/images/main/cart-add-icon.png"></div><div>Add to Cart</div>`);
                }
                else {
                    alert(data.message);
                    $('.cart-'+ productId).html(`Add To Cart`);
    ...
    function addToCartCheckout(productId, categoryId, quantity, price) {
        $('.cart-'+ productId).html(`<i class="fa fa-circle-o-notch fa-spin fa-fw"></i> Loading...`);
        let discount = 0;

        if (productId == 24 && price == 233100) {
            price = 259000;
            discount = 25900;
        }
    ...
    function addToCartReseller(productId, categoryId, quantity, price) {
        $('.cart-'+ productId).html(`<i class="fa fa-circle-o-notch fa-spin fa-fw"></i> Loading...`);
        let discount = 0;
