...
<html>
	...
	<body>
		...
		<div class="container">
			<div class="ui breadcrumb">
				...
				<div class="divider"> / </div>
				<div class="active section">Add CRM</div>
			</div>
		</div>
			...
			<div class="ui styled fluid accordion form form-accordion">
				...
				<div class="active content">
					<div class="ui transition visible form">
					    ...
					    <div class="field">
					      	<label>Answer</label>
					        <input type="text" class="form-input" name="answer" placeholder="Answer" data-accordion-idx="0">
				      	</div>
	...
	<script type="text/javascript">
		...
		function reset() {
			...
			$('#type').dropdown('set selected', `Inquiries`);
			$('#status').dropdown('set selected', `Ongoing`);
			...
			$.ajax({
				...
				success: function(data){
					if (data.status == 'success') {
						$('.success-add').parent().attr('href', '<?= base_url(); ?>crm/add/<?= $patient->id; ?>/').show();
