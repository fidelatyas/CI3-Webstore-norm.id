...
<html>
	...
	<body>
		...
		<div class="container">
			<div class="ui breadcrumb">
				...
				<div class="divider"> / </div>
				<div class="active section">Edit User (<?= $user->name; ?>)</div>
			</div>
		</div>
						...
						<div class="two fields">
							<div class="required field">
					      	...
					      	<div class="field">
						        ...
						        <input type="text" class="form-input" name="password" placeholder="Password" data-accordion-idx="1">
					      	</div>
						</div>

						<div class="two fields">
					    	<div class="field">
					    		<button class="main small red font-10" onclick="updateForm(4);">Previous</button>
					    	</div>
					    	<div class="field text-right">
	...
	<script type="text/javascript">
		...
		function reset() {
			$('input[name="number"]').val(`<?= $user->number; ?>`);
			$('input[name="name"]').val(`<?= $user->name; ?>`);
			$('#status').dropdown('set selected', `<?= $user->status; ?>`);
			$('input[name="position"]').val(`<?= $user->position; ?>`);
			$('input[name="email"]').val(`<?= $user->email; ?>`);
			$('input[name="phone"]').val(`<?= $user->phone; ?>`);
			$('input[name="address"]').val(`<?= $user->address; ?>`);
			$('input[name="username"]').val(`<?= $user->username; ?>`);
			$('#type').dropdown('set selected', `<?= $user->type; ?>`);
			...
			$.ajax({
				...
				success: function(data){
					if (data.status == 'success') {
						$('.success-add').parent().attr('href', '<?= base_url(); ?>user/add/').hide();
