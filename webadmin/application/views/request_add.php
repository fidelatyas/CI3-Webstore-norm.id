...
<html>
					...
					<div class="ui transition visible form">
						...
						<div class="field">
							<table class="ui compact table table-item">
								...
								<? if ($order_id > 0): ?>
									<tbody class="product-list" data-subtotal="<?= $order->subtotal; ?>" data-shipping="<?= $order->shipping; ?>">
										<? foreach ($order->arr_order_item as $key => $order_item): ?>
											<tr id="product-<?= $key + 1; ?>" data-product-id="<?= $order_item->product_id; ?>" data-product-weight="<?= $order_item->product_weight; ?>" data-product-price="<?= $order_item->price; ?>" data-product-qty="<?= $order_item->quantity; ?>" data-product-total="<?= $order_item->total; ?>">
	...
	<script type="text/javascript">
		...
		let productCount = <?= ($order_id > 0) ? count($order->arr_order_item) : 0; ?>;
		...
		function reset() {
			<? if ($order_id > 0): ?>
				$('input[name="name"]').val(`<?= $order->description; ?>`);
				...
				$('#province').dropdown('set selected', `<?= $order->province_id; ?>`);
				$('#city').dropdown('set selected', `<?= $order->city_id; ?>`);
				$('#district').dropdown('set selected', `<?= $order->district_id; ?>`);
			<? else: ?>
				$('input').val(``);
