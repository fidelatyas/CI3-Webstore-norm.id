...
<html>
	...
	<body>
		...
		<div class="container">
			<div class="ui breadcrumb">
				...
				<div class="divider"> / </div>
				<div class="active section">Edit Doctor</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function reset() {
			$('input[name="number"]').val(`<?= $doctor->number; ?>`);
			$('input[name="name"]').val(`<?= $doctor->name; ?>`);
			$('input[name="email"]').val(`<?= $doctor->email; ?>`);
			$('input[name="phone"]').val(`<?= $doctor->phone; ?>`);
			$('input[name="username"]').val(`<?= $doctor->username; ?>`);
			$('#category').dropdown('set selected', [<? foreach ($doctor->arr_category_id as $category_id): ?>'<?= $category_id ?>',<? endforeach; ?>]);
			...
			$.ajax({
				...
				success: function(data){
					if (data.status == 'success') {
						$('.success-add').parent().attr('href', '<?= base_url(); ?>doctor/add/').hide();
				...
				type: 'POST',
				url: '<?= base_url() ?>doctor/ajax_edit/<?= $doctor->id; ?>/',
			});
		}
	</script>
</html>