...
<html>
	...
	<script type="text/javascript">
		...
		function reset() {
			$('input[name="number"]').val(`<?= $points->number; ?>`);
			$('input[name="description"]').val(`<?= $points->description; ?>`);
			$('input[name="points"]').val(`<?= $points->points_display; ?>`);
			$('#status').dropdown(`set selected`, `<?= $points->status; ?>`);
			...
			$.ajax({
				...
				success: function(data){
					if (data.status == 'success') {
						$('.success-add').parent().attr('href', '<?= base_url(); ?>points/add/<?= $patient->id; ?>/').hide();
						$('.success-back').parent().attr('href', '<?= base_url(); ?>points/all/<?= $patient->id; ?>/');
						const message = 'You have successfully edit <?= $patient->name; ?> points to your own database.';
				...
				type: 'POST',
				url: '<?= base_url() ?>points/ajax_edit/<?= $points->id; ?>/',
			});
		}
	</script>
</html>